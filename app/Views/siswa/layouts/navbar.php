<nav class="w-full bg-[#6C7CF6] px-16 py-4 flex flex-col z-30">
    <div class="flex justify-between items-end w-full">
        <button class="block xl:hidden mr-3 text-2xl text-white focus:outline-none"
            id="sidebarToggleBtn"
        ...
        <div>
            ...
            <h6 class="font-bold text-white text-lg capitalize leading-tight mt-0">
                <?= isset($breadcrumb) ? esc(end($breadcrumb)) : 'Dashboard' ?>
            </h6>
        </div>
    ...
    placeholder="Cari jadwal, nilai, absensi ...."
    autocomplete="off"
/>
<div id="searchResults" class="absolute bg-white shadow-md rounded-lg mt-2 w-full z-50 hidden max-h-60 overflow-auto"></div>
            ...
            <?php if (!session('isLoggedIn')): ?>
                <a href="<?= base_url('login') ?>" class="flex items-center gap-1 px-3 py-2 rounded-lg text-white font-semibold hover:bg-white/10 transition text-sm">
                    <i class="fa-solid fa-user"></i>
                    <span class="hidden sm:block">Sign In</span>
                </a>
            <?php else: ?>
                <div class="flex items-center gap-1 px-3 py-2 rounded-lg bg-white/20 text-white font-semibold hover:bg-white/30 transition text-sm">
                    <i class="fa-solid fa-user"></i>
                ...
                <div class="relative">
                    ...
                    <div id="settingsDropdown" class="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-lg py-3 hidden z-50 border border-gray-100">
                        ...
                        <a href="<?= base_url('siswa/profile') ?>" class="flex items-center px-5 py-2 text-gray-700 hover:bg-gray-100 rounded-xl transition">
