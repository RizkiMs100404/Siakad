...
<div class="max-w-xl mx-auto bg-white p-8 rounded-2xl shadow-xl mt-10 border border-blue-100">
    ...
    <form action="<?= base_url('admin/siswa/store') ?>" method="post" class="space-y-5">
        <?= csrf_field() ?>
       <div>
        ...
        <div class="flex gap-3">
            <div class="flex-1">
                ...
                <input type="text" name="nis" value="<?= old('nis') ?>"
                    class="w-full border border-gray-300 rounded-xl px-4 py-2 focus:border-[#6C7CF6] focus:ring-2 focus:ring-[#6C7CF6]/30 transition <?= (session('errors.nis') ? 'border-red-400' : '') ?>">
            </div>
            <div class="flex-1">
                <label class="block text-sm font-medium text-gray-700 mb-1 flex items-center gap-1">
                ...
                <input type="text" name="nisn" value="<?= old('nisn') ?>"
                    class="w-full border border-gray-300 rounded-xl px-4 py-2 focus:border-[#6C7CF6] focus:ring-2 focus:ring-[#6C7CF6]/30 transition <?= (session('errors.nisn') ? 'border-red-400' : '') ?>">
            </div>
        </div>
        <div class="flex gap-3">
            <div class="flex-1">
                ...
                <select name="kelas_id"
                        class="w-full border border-gray-300 rounded-xl px-4 py-2 bg-white focus:border-[#6C7CF6] focus:ring-2 focus:ring-[#6C7CF6]/30 transition <?= (session('errors.kelas_id') ? 'border-red-400' : '') ?>">
            ...
            <div class="flex-1">
                ...
                <input type="text" name="jurusan" value="<?= old('jurusan') ?>"
                    class="w-full border border-gray-300 rounded-xl px-4 py-2 focus:border-[#6C7CF6] focus:ring-2 focus:ring-[#6C7CF6]/30 transition <?= (session('errors.jurusan') ? 'border-red-400' : '') ?>">
            </div>
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1 flex items-center gap-1">
