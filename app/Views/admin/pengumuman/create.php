...
<div class="max-w-xl mx-auto bg-white p-8 rounded-2xl shadow-lg mt-8">
    ...
    <form action="<?= base_url('admin/pengumuman/store') ?>" method="post" class="space-y-4">
        <?= csrf_field() ?>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Judul</label>
            <input type="text" name="judul" value="<?= esc(old('judul')) ?>" autofocus
        ...
        <div>
            ...
            <textarea name="isi" rows="4" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring focus:ring-blue-100"><?= esc(old('isi')) ?></textarea>
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal</label>
            <input type="date" name="tanggal" value="<?= esc(old('tanggal', date('Y-m-d'))) ?>"
                class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring focus:ring-blue-100">
        </div>
        <div class="flex justify-between mt-6">
            ...
            <button type="submit" class="inline-block px-6 py-2 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 transition">Simpan</button>
        </div>
    </form>
</div>
<?= $this->endSection() ?>
