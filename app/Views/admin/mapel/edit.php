...
<div class="max-w-xl mx-auto bg-white p-8 rounded-2xl shadow-xl mt-10 border border-amber-100">
    ...
    <form action="<?= base_url('admin/mapel/update/'.$mapel['id']) ?>" method="post" class="space-y-5">
        <?= csrf_field() ?>
        <div>
            ...
            <input type="text" name="nama_mapel" value="<?= old('nama_mapel', $mapel['nama_mapel']) ?>"
                   class="w-full border border-gray-300 rounded-xl px-4 py-2 focus:border-amber-500 focus:ring-2 focus:ring-amber-100/40 transition <?= (session('errors.nama_mapel') ? 'border-red-400' : '') ?>">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1 flex items-center gap-1">
