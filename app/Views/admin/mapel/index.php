...
<div class="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-3">
    <h2 class="text-2xl font-bold text-slate-700 flex items-center gap-2">
        <i class="fa-solid fa-book-open text-[#6C7CF6]"></i> <?= esc($pageTitle ?? 'Data Mapel') ?>
    </h2>
    <a href="<?= base_url('admin/mapel/create') ?>"
...
<div class="mb-4 flex flex-col md:flex-row md:items-center gap-3">
    <form method="get" class="flex flex-1 items-center gap-2">
        <div class="relative w-full max-w-xs">
            <input type="text" name="search" value="<?= esc($_GET['search'] ?? '') ?>"
                placeholder="Cari Mapel atau Guru ID..." 
    ...
    <a href="<?= base_url('admin/mapel') ?>" class="text-sm text-blue-500 hover:underline">Reset</a>
</div>
    ...
    <table class="min-w-full table-auto text-sm text-slate-700">
        <thead>
            <tr class="bg-[#6C7CF6]/80 text-white">
                ...
                <th class="p-3 text-left">Guru Pengampu</th>
        ...
        <tbody>
            <?php foreach($mapel as $i => $m): ?>
                <tr class="border-b hover:bg-blue-50/50 group">
                    ...
                    <td class="p-3"><?= esc($m['nama_mapel']) ?></td>
                    <td class="p-3">
                        <?php if (!empty($m['nama_guru'])): ?>
                            <span class="inline-flex items-center gap-2 px-3 py-1 rounded bg-blue-100 text-blue-700 font-semibold text-xs">
                                <i class="fa-solid fa-user-tie text-blue-500"></i> <?= esc($m['nama_guru']) ?>
                            </span>
                        <?php else: ?>
                    ...
                    <td class="p-3 text-center flex flex-row gap-2 justify-center">
                        <a href="<?= base_url('admin/mapel/edit/'.$m['id']) ?>"
                           class="inline-block px-3 py-1 rounded bg-amber-400 text-white hover:bg-amber-500 transition"
                           title="Edit"><i class="fa-solid fa-pen-to-square"></i></a>
                        <form action="<?= base_url('admin/mapel/delete/'.$m['id']) ?>" method="post" class="inline">
                            <?= csrf_field() ?>
                            <button type="submit" onclick="return confirm('Yakin hapus mapel ini?')" 
            ...
            <?php endforeach ?>
            <?php if (empty($mapel)): ?>
                <tr><td colspan="4" class="p-6 text-center text-gray-400">Belum ada data mapel.</td></tr>
            <?php endif ?>
        </tbody>
    </table>
</div>
