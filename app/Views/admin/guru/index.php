...
<div class="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-3">
    <h2 class="text-2xl font-bold text-slate-700 flex items-center gap-2">
        <i class="fa-solid fa-chalkboard-user text-[#6C7CF6]"></i> <?= esc($pageTitle ?? 'Data Guru') ?>
    </h2>
...
<div class="mb-4 flex flex-col md:flex-row md:items-center gap-3">
    ...
    <a href="<?= base_url('admin/guru') ?>" class="text-sm text-blue-500 hover:underline">Reset</a>
</div>
...
<tbody>
    <?php foreach($guru as $i => $g): ?>
        <tr class="border-b hover:bg-blue-50/50 group">
            <td class="p-3"><?= $i+1 ?></td>
            <td class="p-3">
                <?php if (!empty($g['user_id']) && !empty($users[$g['user_id']])): ?>
                    <span class="inline-block px-3 py-1 rounded-full bg-blue-100 text-blue-800 text-xs font-semibold shadow border border-blue-200">
                        <?= esc($users[$g['user_id']]['username']) ?>
                    </span>
                <?php else: ?>
                    <span class="inline-block px-2 py-1 rounded bg-gray-100 text-gray-500 text-xs">-</span>
                <?php endif ?>
            </td>
            <td class="p-3"><?= esc($g['nama_lengkap']) ?></td>
            <td class="p-3"><?= esc($g['nip']) ?></td>
            <td class="p-3"><?= esc($g['mapel']) ?></td>
            <td class="p-3"><?= esc($g['no_hp']) ?></td>
            <td class="p-3"><?= esc($g['alamat']) ?></td>
            <td class="p-3 text-center flex flex-row gap-2 justify-center">
                ...
                <form action="<?= base_url('admin/guru/delete/'.$g['id']) ?>" method="post" class="inline">
                    <?= csrf_field() ?>
                    <button type="submit" onclick="return confirm('Yakin hapus guru ini?')" 
    ...
    <?php endforeach ?>
    <?php if (empty($guru)): ?>
        <tr><td colspan="8" class="p-6 text-center text-gray-400">Belum ada data guru.</td></tr>
    <?php endif ?>
</tbody>
